...
const SilaiCompleteOrders = () => {
  const [list] = useState<SilaiItemType[]>([
    {
  ...
  const renderItemHandler = useCallback(({item}: {item: SilaiItemType}) => {
    return <SilaiItems data={item} />;
  }, []);
...
export default memo(SilaiCompleteOrders);
