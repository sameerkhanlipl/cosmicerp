...
import { packing_search_orders } from "../../api/apis";
import { packing_search_order_body } from "../../api/BodyTypes";
import { packing_search_order_listing_response } from "../../api/ResponseTypes";
...
import PackingSearchOrderItem, {
  PackingSearchOrderItemType
} from "./PackingSearchOrderItem";
...
const PackingSearchOrders = () => {
  const [list, setList] = useState<PackingSearchOrderItemType[]>([]);
  ...
  const getList = useCallback(async () => {
    ...
    try {
      setLoader(true);
      const body: packing_search_order_body = {
        search: search?.current?.get()
      };
      const response: { data: packing_search_order_listing_response } =
        await packing_search_orders(body);
  ...
  const renderItemHandler = useCallback(
    ({ item }: { item: PackingSearchOrderItemType }) => {
      return (
        <PackingSearchOrderItem
          onPress={onNavigatePackingOrderHistory}
          data={item}
        />
      );
    },
  ...
  return (
    <View style={styles.root}>
      <CommonHeader title={"Search Order Of Packing"} icon={images.close} />
      <Input
        ref={search}
        label=""
        config={{
